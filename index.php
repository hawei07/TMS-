...
function handleApi() {
    ...
    if (dispatchExtractedApi($db, $action, $method, $_GET, $input)) {
        return;
    }
...
// ==================== 活动管理 API ====================
        ...
        case 'save_activity':
            ...
            $adult_fee_mode = trim((string)($input['adult_fee_mode'] ?? ''));
            $student_fee_mode = trim((string)($input['student_fee_mode'] ?? ''));
...
// ==================== 员工搜索 API ====================
        case 'search_employees':
            $keyword = trim($_GET['keyword'] ?? ($input['keyword'] ?? ''));
            $campus = trim($_GET['campus'] ?? ($input['campus'] ?? ''));
...
// ==================== 价格管理 API ====================
        case 'pay_enroll':
            ...
            foreach ($items as $i => $item) {
                ...
                $stmt->bindValue(':coa', floatval($di['cp_amount'] ?? 0), PDO::PARAM_STR);
                ...
                $totalLessons += intval($item['lesson_count']) + intval($item['gifted_lessons'] ?? 0);
            }
            // 查学员信息
...
// ==================== 学科设置 API ====================
        ...
        case 'get_student':
            ...
            $summary = $db->query("SELECT
                ...
                COALESCE(SUM(actual_price - COALESCE(teaching_aid_price,0) + COALESCE(product_coupon_amount,0)), 0) AS total_amount,
                COALESCE(SUM((actual_price - COALESCE(teaching_aid_price,0) + COALESCE(product_coupon_amount,0)) * consumed_lessons / NULLIF(lesson_count, 0)), 0) AS consumed_amount
        ...
        case 'get_student_courses':
            ...
            foreach ($orderRows as $r) {
                ...
                $giftedLessons = intval($r['gifted_lessons'] ?? 0);
                ...
                // 付费课消耗（不超过付费课时数）
                $paidConsumed = min($realConsumed, $lc);
                // 赠课消耗（超出付费课时的部分）
                $giftConsumed = max(0, $realConsumed - $lc);
                ...
                if ($giftedLessons > 0) {
                    $giftRow = [
                        'id' => $r['id'],
                        'name' => $r['name'],
                        'subject_level1' => $r['subject_level1'],
                        'subject_level2' => $r['subject_level2'],
                        'plan_name' => $r['plan_name'],
                        'item_name' => ($r['item_name'] ?? '') . '（赠送）',
                        'lesson_count' => $giftedLessons,
                        'actual_price' => 0,
                        ...
                        'consumed_lessons' => min($giftConsumed, $giftedLessons),
                        'campus' => $r['campus'],
                        ...
                        'refunded_lessons' => 0,
                        'remaining_lessons' => max(0, $giftedLessons - $giftConsumed),
                        'remaining_amount' => 0,
                    ...
                    // 退费/退费申请中：赠课一并标记
                    if ($refundStatus === '已退费') {
                        $giftRow['remaining_lessons'] = 0;
                        $giftRow['consumed_lessons'] = $giftedLessons;
                        $giftRow['refunded_lessons'] = max(0, $giftedLessons - $giftConsumed);
                    } elseif (isset($pendingRefundIds[$r['order_id']])) {
                        $giftRow['remaining_lessons'] = 0;
                    }
                    $rows[] = $giftRow;
            ...
            while ($tr = $trStmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $row = [
                    ...
                    'consumed_amount' => $transferAmount - $remainingAmount,
                    'refunded_lessons' => 0,
            ...
            while ($ctrRow = $ctrStmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $rows[] = [
                    'id' => (int)$ctrRow['target_course_id'],
                    'name' => $tCourse['name'] ?? $ctrRow['target_course_name'],
                    'subject_level1' => $tCourse['subject_level1'] ?? '',
                    'subject_level2' => $tCourse['subject_level2'] ?? '',
                    'plan_name' => '转课',
                    'item_name' => '转课课程',
            ...
            $ctrCourseIds = [];
            foreach ($rows as $r) {
                if (!empty($r['_ctr_target_cid'])) {
                    $ctrIds[] = (int)$r['_ctr_target_cid'];
                    $ctrCourseIds[] = (int)$r['_ctr_target_cid'];
                }
            }
            ...
            usort($rows, function ($a, $b) {
                ...
                return ($a['_sort_index'] ?? 0) <=> ($b['_sort_index'] ?? 0);
            });
            foreach ($rows as &$row) {
                unset($row['_sort_index']);
            }
            unset($row);
            json(['data' => $rows]);
            break;
...
// ==================== 退费记录 API ====================
        ...
        case 'submit_refund':
            ...
            if ($refundType === 'course') {
                ...
                if ($transferId > 0) {
                    // 转校课包退费
                    $tr = $db->query("SELECT * FROM transfer_records WHERE id=$transferId AND status='已通过'")->fetch(PDO::FETCH_ASSOC);
                    if (!$tr) { json(['error' => '转校记录不存在或未通过审批']); break; }
                    if (($tr['refund_status'] ?? '正常') !== '正常') { json(['error' => '该转校课包已申请退费']); break; }

                    $lessonCount = intval($tr['transfer_lessons'] ?? 0);
                    $transferAmount = floatval($tr['transfer_amount'] ?? 0);
                ...
                $giftedLessons = intval($order['gifted_lessons'] ?? 0);
                $consumedLessons = intval($order['consumed_lessons'] ?? 0);
                if ($consumedLessons >= ($lessonCount + $giftedLessons)) { json(['error' => '该课程已全部消耗（含赠课），无法退费']); break; }
                if ($consumedLessons >= $lessonCount && $giftedLessons <= 0) { json(['error' => '该课程已全部消耗，无法退费']); break; }
        ...
        case 'approve_refund':
            ...
            if ($currentStage === '一级审批') {
                ...
                if ($rrProject === '课程' && $rrMethod === '账户') {
                    ...
                    $order2 = $db->query("SELECT lesson_count, gifted_lessons FROM orders WHERE id=$orderId")->fetch(PDO::FETCH_ASSOC);
                    $lc = $order2 ? intval($order2['lesson_count']) : 0;
                    $gl = $order2 ? intval($order2['gifted_lessons'] ?? 0) : 0;
                    $db->beginTransaction();
                    try {
                        ...
                        // 2. 将订单消耗课时设置为总课时（含赠课，剩余课时归零）
                        $db->exec("UPDATE orders SET refund_status='已退费', consumed_lessons=" . ($lc + $gl) . " WHERE id=$orderId");
        ...
        case 'submit_transfer':
            ...
            try {
                if ($transferId > 0) {
                    // 转校课包再次转校
                    $tr = $db->query("SELECT * FROM transfer_records WHERE id=$transferId AND status='已通过' FOR UPDATE")->fetch(PDO::FETCH_ASSOC);
                    if (!$tr) {
                        $db->rollBack();
                        json(['error' => '转校记录不存在或未通过审批']);
                        break;
                    }
                    if ($tr['to_campus'] === $toCampus) {
                        $db->rollBack();
                        json(['error' => '目标校区不能与当前校区相同']);
                        break;
                    }
                    $totalTransferred = intval($tr['total_transferred'] ?? 0);
                    $available = intval($tr['transfer_lessons']) - $totalTransferred;
                    ...
                    $unitPrice = intval($tr['transfer_lessons']) > 0 ? floatval($tr['transfer_amount']) / intval($tr['transfer_lessons']) : 0;
                    $transferAmount = round($unitPrice * $transferLessons, 2);
                    $n = now();

                    $stmt = $db->prepare("INSERT INTO transfer_records (order_id, source_transfer_id, student_id, student_name, order_no, course_id, course_name, subject_level1, subject_level2, plan_name, item_name, from_campus, to_campus, transfer_lessons, transfer_amount, original_remaining_lessons, status, applicant, created_at) VALUES (:oid, :stid, :sid, :sname, :ono, :cid, :cname, :sl1, :sl2, :pn, :inm, :fc, :tc, :tl, :ta, :orl, '待审批', :app, :ct)");
                    $stmt->bindValue(':oid', 0, PDO::PARAM_INT);
                    ...
                    $stmt->bindValue(':pn', $tr['plan_name'] ?? '', PDO::PARAM_STR);
                    $stmt->bindValue(':inm', $tr['item_name'] ?? '', PDO::PARAM_STR);
                    $stmt->bindValue(':fc', $tr['to_campus'] ?? '', PDO::PARAM_STR);
                    $stmt->bindValue(':tc', $toCampus, PDO::PARAM_STR);
                    $stmt->bindValue(':tl', $transferLessons, PDO::PARAM_INT);
                    $stmt->bindValue(':ta', $transferAmount);
                    $stmt->bindValue(':orl', $available, PDO::PARAM_INT);
                    $stmt->bindValue(':app', $applicant, PDO::PARAM_STR);
                    $stmt->bindValue(':ct', $n, PDO::PARAM_STR);
                    $stmt->execute();
                ...
                if ($order['campus'] === $toCampus) {
                    $db->rollBack();
                    json(['error' => '目标校区不能与当前校区相同']);
                    break;
                }
                $lc = intval($order['lesson_count'] ?? 0);
                $consumed = intval($order['consumed_lessons'] ?? 0);
                $transferred = intval($order['transferred_lessons'] ?? 0);
                $available = $lc - $consumed - $transferred;
                if ($transferLessons > $available) {
                    $db->rollBack();
                    json(['error' => "可转移课时不足，剩余可用 {$available} 课时"]);
                    break;
                ...
                $n = now();
                $stmt = $db->prepare("INSERT INTO transfer_records (order_id, student_id, student_name, order_no, course_id, course_name, subject_level1, subject_level2, plan_name, item_name, from_campus, to_campus, transfer_lessons, transfer_amount, original_remaining_lessons, status, applicant, created_at) VALUES (:oid, :sid, :sname, :ono, :cid, :cname, :sl1, :sl2, :pn, :inm, :fc, :tc, :tl, :ta, :orl, '待审批', :app, :ct)");
                $stmt->bindValue(':oid', $orderId, PDO::PARAM_INT);
                $stmt->bindValue(':sid', intval($order['student_id']), PDO::PARAM_INT);
                $stmt->bindValue(':sname', $order['student_name'] ?? '', PDO::PARAM_STR);
                $stmt->bindValue(':ono', $order['order_no'] ?? '', PDO::PARAM_STR);
                $stmt->bindValue(':cid', intval($order['course_id'] ?? 0), PDO::PARAM_INT);
                $stmt->bindValue(':cname', $order['course_name'] ?? '', PDO::PARAM_STR);
                $stmt->bindValue(':sl1', $order['course_sl1'] ?? '', PDO::PARAM_STR);
                $stmt->bindValue(':sl2', $order['course_sl2'] ?? '', PDO::PARAM_STR);
                $stmt->bindValue(':pn', $order['plan_name'] ?? '', PDO::PARAM_STR);
                $stmt->bindValue(':inm', $order['item_name'] ?? '', PDO::PARAM_STR);
                $stmt->bindValue(':fc', $order['campus'] ?? '', PDO::PARAM_STR);
                $stmt->bindValue(':tc', $toCampus, PDO::PARAM_STR);
                $stmt->bindValue(':tl', $transferLessons, PDO::PARAM_INT);
                $stmt->bindValue(':ta', $transferAmount);
                $stmt->bindValue(':orl', $available, PDO::PARAM_INT);
                ...
                $db->rollBack();
                json(['error' => '提交失败：' . $e->getMessage()]);
            }
            break;

        case 'list_transfer_records':
            $where = ['1=1'];
            $status = trim($_GET['status'] ?? '');
            ...
            if ($search !== '') {
                ...
                $where[] = "(student_name LIKE $like OR order_no LIKE $like OR course_name LIKE $like)";
            }
            $whereSql = implode(' AND ', $where);
            $page = max(1, intval($_GET['page'] ?? 1));
            $pageSize = min(50, max(1, intval($_GET['page_size'] ?? 20)));
            $offset = ($page - 1) * $pageSize;
            $countStmt = $db->query("SELECT COUNT(*) AS total FROM transfer_records WHERE $whereSql");
            $total = intval($countStmt->fetch(PDO::FETCH_ASSOC)['total']);
            $rows = $db->query("SELECT * FROM transfer_records WHERE $whereSql ORDER BY id DESC LIMIT $offset, $pageSize")->fetchAll(PDO::FETCH_ASSOC);
            json(['data' => $rows, 'total' => $total, 'page' => $page, 'page_size' => $pageSize]);
            break;
        ...
        case 'approve_transfer':
            ...
            $db->beginTransaction();
            try {
                $tr = $db->query("SELECT * FROM transfer_records WHERE id=$id FOR UPDATE")->fetch(PDO::FETCH_ASSOC);
                if (!$tr) {
                    $db->rollBack();
                    json(['error' => '转校记录不存在']);
                    break;
                ...
                if ($tr['status'] !== '待审批') {
                    $db->rollBack();
                    json(['error' => '该申请已处理，无法重复审批']);
                    break;
                }

                $n = now();
                if ($action === 'reject') {
                    $db->exec("UPDATE transfer_records SET status='已驳回', reject_reason=" . $db->quote($rejectReason) . ", approver=" . $db->quote($approver) . ", updated_at='$n' WHERE id=$id");
                    $db->commit();
                    json(['message' => '已驳回转校申请']);
                    break;
                }
...
// ==================== 班级学员管理 API ====================
        ...
        case 'get_available_students':
            ...
            if (trim($subjectLevel1) !== '') {
                ...
                $sql = "SELECT s.id, s.student_no, s.name, s.phone,
                    COALESCE((SELECT SUM(o.lesson_count + COALESCE(o.gifted_lessons, 0) - o.consumed_lessons)
                        FROM orders o
                        ...
                        AND (o.lesson_count + COALESCE(o.gifted_lessons, 0)) > o.consumed_lessons
        ...
        case 'get_temp_student_candidates':
            ...
            $sql = "SELECT s.id, s.student_no, s.name, s.phone, SUM(o.lesson_count + COALESCE(o.gifted_lessons, 0) - o.consumed_lessons) AS remaining
                FROM students s
            ...
            try {
                foreach ($records as $rec) {
                    ...
                    if (is_array($oldEntries2) && !empty($oldEntries2)) {
                        $oldOrderIds2 = array_filter(array_map('intval', array_column($oldEntries2, 'order_id')), function($oid) { return $oid > 0; });
                        $oldDeductedOrderIds = array_values($oldOrderIds2);
                        $oidList2 = implode(',', $oldOrderIds2);
                        $refundBlock = !empty($oidList2) ? $db->query("SELECT order_id, status FROM refund_records WHERE order_id IN ($oidList2) AND status IN ('待审批', '一级审批通过', '二级审批通过', '已退费') LIMIT 1")->fetch(PDO::FETCH_ASSOC) : null;
                        if ($refundBlock) {
                    ...
                    if ($oldAtt && !empty($oldAtt['deduction_json'])) {
                        ...
                        if (is_array($oldEntries)) {
                            foreach ($oldEntries as $entry) {
                                ...
                                if ($tid > 0 && $amt > 0) {
                                    // 归还转课记录课时
                                    $db->exec("UPDATE course_transfer_records SET consumed_lessons = consumed_lessons - $amt WHERE id = $tid");
                                    $logLine .= "REVERTED transfer_record=$tid amt=$amt\n";
                                } else if ($oid > 0 && $amt > 0) {
                    ...
                    if ($status === '出勤' && $deductedLessons > 0) {
                        ...
                        if ($remainingToDeduct > 0) {
                            ...
                            while ($tr = $tRes->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                $db->exec("UPDATE course_transfer_records SET consumed_lessons = consumed_lessons + $toDeduct WHERE id = " . intval($tr['id']));
                                $deductionEntries[] = ['transfer_record_id' => $tr['id'], 'amount' => $toDeduct];
                                $remainingToDeduct -= $toDeduct;
                            }
                        }
                        if ($remainingToDeduct > 0) {
                    ...
                    if (!empty($deductionEntries)) {
                        foreach ($deductionEntries as $de) {
                            $deOrderId = intval($de['order_id'] ?? 0);
                            $deTransferId = intval($de['transfer_record_id'] ?? 0);
                            $amt = floatval($de['amount']);
                            // 转课记录：从转课记录自身计算目标课时单价
                            if ($deTransferId > 0) {
                                $trRow = $db->query("SELECT source_order_id, target_lessons, target_value FROM course_transfer_records WHERE id = $deTransferId")->fetch(PDO::FETCH_ASSOC);
                                if ($trRow && intval($trRow['target_lessons']) > 0) {
                                    $consumedAmount += (floatval($trRow['target_value']) / intval($trRow['target_lessons'])) * $amt;
                                }
                                continue;
                            }
                            if ($deOrderId <= 0) continue;
                            $orderRow = $db->query("SELECT actual_price, teaching_aid_price, product_coupon_amount, lesson_count FROM orders WHERE id={$deOrderId}")->fetch(PDO::FETCH_ASSOC);
...
json([
        ...
        case 'list_activity_attendance':
            ...
            while ($r = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                if ($att) {
                    ...
                    $r['attendance_time'] = $att['created_at'] ?? '';
                    $r['teacher'] = $att['teacher'] ?? '';
                    // 兜底：如果teacher列为空，从deduction_breakdown提取
                    if (empty($r['teacher'])) {
                        $breakdown = json_decode($att['deduction_breakdown'] ?? '{}', true) ?: [];
                        $r['teacher'] = $breakdown['teacher'] ?? '';
                    }
        ...
        case 'save_activity_attendance':
            ...
            // 提取带课老师
            $teacher = '';
            if ($deductionBreakdownRaw) {
                $bd = is_array($deductionBreakdownRaw) ? $deductionBreakdownRaw : json_decode($deductionBreakdownRaw, true);
                $teacher = ($bd && isset($bd['teacher'])) ? trim($bd['teacher']) : '';
            }
            ...
            // 获取 attendance_id（修改考勤时前端传入）
            $attendanceId = intval($input['attendance_id'] ?? 0);
            if ($attendanceId <= 0) {
                $existingAtt = $db->query("SELECT id FROM class_attendance WHERE activity_id = $activityId AND activity_order_id = $activityOrderId LIMIT 1")->fetch(PDO::FETCH_ASSOC);
                if ($existingAtt) $attendanceId = intval($existingAtt['id']);
            }
            ...
            try {
                if ($attendanceId > 0) {
                    // 修改考勤：先归还旧扣课
                    $oldAtt = $db->query("SELECT * FROM class_attendance WHERE id = $attendanceId")->fetch(PDO::FETCH_ASSOC);
                    if ($oldAtt) {
                        $oldJson = $oldAtt['deduction_json'] ?? '[]';
                        $oldEntries = json_decode($oldJson, true) ?: [];
                        foreach ($oldEntries as $entry) {
                            $oid = intval($entry['order_id'] ?? 0);
                            $amt = intval($entry['amount'] ?? 0);
                            if ($oid > 0 && $amt > 0) {
                        ...
                        if (empty($oldEntries)) {
                            $oldDl = intval($oldAtt['deducted_lessons'] ?? 0);
                            $oldDoid = intval($oldAtt['deducted_order_id'] ?? 0);
                            if ($oldDoid > 0 && $oldDl > 0) {
                                $db->exec("UPDATE orders SET consumed_lessons = GREATEST(0, consumed_lessons - $oldDl) WHERE id = $oldDoid");
                            }
                        }
                    }
                    // 归还后重新读取 srcConsumed 确保剩余课时校验准确
                    $srcConsumed = intval($db->query("SELECT consumed_lessons FROM orders WHERE id = $deductOrderId")->fetchColumn() ?? 0);
                    $remaining = ($srcLc + $srcGl) - $srcConsumed;
                    if ($remaining <= 0) { $db->rollBack(); json(['error' => '该课包已无剩余课时']); break; }
                    if ($totalDeductLessons > $remaining) { $db->rollBack(); json(['error' => '扣课时数(' . $totalDeductLessons . ')超过剩余课时(' . $remaining . ')']); break; }
                    // 重新计算 consumedAmount（基于最新 srcConsumed 价格不变）
                }
        ...
        case 'list_activity_consumptions':
            ...
            $rows = [];
            while ($r = $stmt->fetch(PDO::FETCH_ASSOC)) {
                // 活动收费已考勤 = 总价 / 报名总人数 × 出勤总人数
                $totalPrice = floatval($r['activity_total_price'] ?? 0);
                $adultCount = intval($r['activity_adult_count'] ?? 0);
                $studentCount = intval($r['activity_student_count'] ?? 0);
                $adultAttended = intval($r['adult_attended'] ?? 0);
                $studentAttended = intval($r['student_attended'] ?? 0);
                $totalPeople = $adultCount + $studentCount;
                $r['activity_attended_fee'] = 0;
                if ($totalPeople > 0 && $totalPrice > 0) {
                    $r['activity_attended_fee'] = round($totalPrice / $totalPeople * ($adultAttended + $studentAttended), 2);
                }
                // 考勤时间
                $r['attendance_time'] = $r['created_at'] ?? '';
                // 解析扣课来源
                ...
                if (!empty($entries)) {
                    $srcOrderId = intval($entries[0]['order_id'] ?? 0);
                    if ($srcOrderId > 0) {
                        $srcOrder = $db->query("SELECT o.item_name, o.order_no, c.subject_level1, c.subject_level2 FROM orders o LEFT JOIN courses c ON o.course_id = c.id WHERE o.id = $srcOrderId")->fetch(PDO::FETCH_ASSOC);
                        if ($srcOrder) {
        ...
        case 'get_campus_courses':
            $campus = trim($_GET['campus'] ?? '');
            $excludeIds = trim($_GET['exclude_ids'] ?? '');
            if (!$campus) { json(['error' => '缺少校区参数']); break; }
            // 校区名称 → 组织ID 转换（campus_permission 存的是ID）
            $campusId = $db->query("SELECT id FROM organizations WHERE name = " . $db->quote($campus) . " AND type='校区'")->fetchColumn();
            if (!$campusId) { json(['data' => []]); break; }
            $sql = "SELECT id, name, subject_level1, subject_level2 FROM courses WHERE FIND_IN_SET(:cid, campus_permission)";
            $params = [':cid' => $campusId];
            if ($excludeIds) {
                $ids = array_map('intval', explode(',', $excludeIds));
                $placeholders = implode(',', array_map(fn($i) => ":exclude_$i", array_keys($ids)));
                $sql .= " AND id NOT IN ($placeholders)";
                foreach ($ids as $i => $id) $params[":exclude_$i"] = $id;
            }
            $sql .= " ORDER BY name";
            $stmt = $db->prepare($sql);
...
<body>
        ...
        <main class="main-content">
                    ...
                    <table class="course-table">
                        <thead>
                            <tr>
                                ...
                                <th class="col-subject">科目分类</th>
                                <th class="col-campus">授权校区</th>
                                <th class="col-bool">小课包</th>
                                <th class="col-bool">低幼龄</th>
                                <th class="col-actions">操作</th>
                            </tr>
                        </thead>
                        <tbody></tbody>
                    </table>
                    <div class="course-table-empty" style="display:none;">暂无课程数据</div>
                </div>
                <div class="pagination" id="pagination-course"></div>
                    ...
                    <div class="sec-panel" id="tab-activities-panel">
                        ...
                        <div id="tab-activity-form" style="display:none;">
                            ...
                            <div class="form-panel-body" style="max-width:750px;">
                                ...
                                <div class="activity-section-wrap">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="activity-name" maxlength="100" placeholder="请输入活动名称" autocomplete="off" data-validate="required">
                                    ...
                                    <div class="form-row">
                                        <div class="form-group form-group-half">
                                            ...
                                            <input type="date" id="activity-reg-start" data-validate="required">
                                            <div class="field-error-msg" style="display:none;"></div>
                                        </div>
                                        <div class="form-group form-group-half">
                                            <label>报名结束日期 <span class="required">*</span></label>
                                            <input type="date" id="activity-reg-end" data-validate="required">
                                            <div class="field-error-msg" style="display:none;"></div>
                                        </div>
                                    </div>
                                </div>

                                <!-- ===== Section 02: 收费设置（列表式双行） ===== -->
                                <div class="activity-section-wrap">
                                    <div class="activity-section-header">
                                        <span class="section-badge">02</span>
                                        <span class="section-label">收费设置</span>
                                        <span class="section-hint">* 成人 / 学员收费各自独立设置</span>
                                    </div>

                                    <div class="fee-list">
                                        <div class="fee-list-head">
                                            <div class="fl-col fl-obj">对象</div>
                                            <div class="fl-col fl-mode">收费模式</div>
                                            <div class="fl-col fl-price">价格 (元)</div>
                                            <div class="fl-col fl-deduct">扣课学科设置</div>
                                        </div>

                                        <!-- 成人行 -->
                                        <div class="fee-list-row">
                                            <div class="fl-col fl-obj"><span class="obj-badge obj-adult">成</span></div>
                                            <div class="fl-col fl-mode">
                                                <select id="activity-adult-fee-mode" class="fee-mode-select" onchange="onActivityFeeModeChange('adult')">
                                                    <option value="">请选择</option>
                                                    <option value="fee_only">仅收费</option>
                                                    <option value="fee_and_deduct">收费 + 扣课</option>
                                                    <option value="deduct_only">仅扣课</option>
                                                </select>
                                            </div>
                                            <div class="fl-col fl-price">
                                                <div class="form-group" id="activity-adult-price-row" style="display:none;margin:0;">
                                                    <input type="number" id="activity-adult-price" min="0" step="0.01" placeholder="0.00">
                                                </div>
                                            </div>
                                            <div class="fl-col fl-deduct">
                                                <div id="activity-adult-deduct-section" style="display:none;">
                                                    <div id="activity-adult-deduct-rows" class="deduct-rows-inline"></div>
                                        ...
                                        <!-- 学员行 -->
                                        <div class="fee-list-row">
                                            <div class="fl-col fl-obj"><span class="obj-badge obj-student">学</span></div>
                                            <div class="fl-col fl-mode">
                                                <select id="activity-student-fee-mode" class="fee-mode-select" onchange="onActivityFeeModeChange('student')">
                                                    <option value="">请选择</option>
                                                    <option value="fee_only">仅收费</option>
                                                    <option value="fee_and_deduct">收费 + 扣课</option>
                                                    <option value="deduct_only">仅扣课</option>
                                            ...
                                            <div class="fl-col fl-price">
                                                <div class="form-group" id="activity-student-price-row" style="display:none;margin:0;">
                                                    <input type="number" id="activity-student-price" min="0" step="0.01" placeholder="0.00">
                                                </div>
                                            </div>
                                            <div class="fl-col fl-deduct">
                                                <div id="activity-student-deduct-section" style="display:none;">
                                                    <div id="activity-student-deduct-rows" class="deduct-rows-inline"></div>
                                ...
                                <div class="activity-section-wrap">
                                    <div class="activity-section-header">
                                        ...
                                        <span class="section-label">适用校区 <span class="required">*</span></span>
                                    </div>

                                    <div class="campus-toolbar">
                                        <div class="campus-search-wrap">
                                            <svg class="campus-search-icon" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                                            <input type="text" class="campus-search-input" placeholder="搜索校区…" id="campus-search-input" oninput="filterActivityCampusTree(this.value)">
                                        </div>
                                        <div class="campus-quick-actions">
                                            <button type="button" class="link-btn" id="campus-select-all">全选</button>
                                            <button type="button" class="link-btn" id="campus-deselect-all">取消全选</button>
                                        </div>
                                    </div>

                                    <div class="campus-tree" id="activity-campus-tree"></div>
                                    <div id="campus-empty-msg" style="display:none;padding:20px;text-align:center;color:#999;font-size:13px;">没有匹配的校区</div>

                                    <div class="campus-batch-cap">
                                        <span>批量填充选中校区报名人数：</span>
                                        <input type="number" id="batch-capacity-value" min="0" placeholder="0">
                                        <button type="button" class="link-btn" id="apply-batch-cap">应用</button>
                                        <span class="campus-summary" id="campus-summary"></span>
                                    </div>
                                </div>
                            ...
                            <div class="activity-form-footer" id="activity-form-footer">
                                <span class="footer-hint" id="footer-validation-hint"></span>
                                <div class="footer-actions">
                                    <button class="btn btn-outline" onclick="backToActivityList()">取消</button>
                                    <button class="btn btn-primary" id="btn-save-activity" onclick="saveActivity()" disabled>发布活动</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <section class="content-panel" id="panel-students">
                ...
                <div class="table-wrap">
                    <table id="table-students">
                        <thead><tr>
                            <th width="70">学号</th><th>姓名</th><th>手机号</th><th width="70" class="col-student-type">学员类型</th><th>校区</th><th>所在班级</th><th>学科剩余课时</th><th>授课老师</th><th width="180">操作</th>
                        </tr></thead>
                        <tbody></tbody>
                    </table>
                </div>
                    ...
                    <div class="att-panel" id="tab-attendance-operations">
                        ...
                        <div class="att-panel" id="tab-activity-attendance">
                            <div class="act-toolbar">
                                <input id="att-activity-search" placeholder="搜索学员/活动...">
                                <select id="att-activity-campus-filter"><option value="">全部校区</option></select>
                                <button class="btn-search" onclick="loadActivityAttendanceList()">查询</button>
                            </div>
                            <div class="table-wrap">
                                <table class="activity-att-table">
                                    <thead><tr>
                                        <th>学员</th><th>手机号</th><th>学号</th><th>活动名称</th><th>报名校区</th><th>报名时间</th><th class="col-right">活动收费</th><th>一级学科</th><th>二级学科</th><th>考勤时间</th><th>带课老师</th><th class="col-center">考勤状态</th><th class="col-center">操作</th>
                                    </tr></thead>
                                    <tbody id="activity-attendance-tbody"><tr><td colspan="13">加载中...</td></tr></tbody>
                                </table>
                            </div>
            ...
            <section class="content-panel" id="panel-enroll">
                ...
                <div class="enroll-action-bar" id="enroll-action-bar">
                    ...
                    <div class="enroll-activity-flow" id="enroll-activity-flow" style="display:none;">
                        ...
                        <!-- 选择活动 — 表格列表 -->
                        <div class="enroll-section" id="enroll-activity-step-activity" style="display:none;">
                            <div class="enroll-section-title">📋 选择活动 <span style="font-weight:400;font-size:12px;color:var(--color-text-muted);margin-left:8px;">· 当前校区可报名</span></div>
                            <div class="activity-search-bar">
                                <div class="ta-filter-group ta-filter-activity-name">
                                    <span class="ta-filter-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg></span>
                                ...
                                <div class="ta-filter-group ta-filter-activity-subject">
                                    <span class="ta-filter-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg></span>
                                    <select id="activity-subject-filter" class="ta-filter-input" onchange="filterActivities()" style="min-width:110px;"><option value="">全部学科</option></select>
                                </div>
                            </div>
                            <table class="enroll-activity-table">
                                <thead><tr><th style="width:22px"></th><th>活动名称</th><th>学科</th><th>费用</th><th>剩余名额</th><th>扣课时</th></tr></thead>
                                <tbody id="activity-card-list"></tbody>
                            </table>
                        ...
                        <!-- 报名人数 — 选活动后展示 -->
                        <div class="enroll-section" id="enroll-activity-step-count" style="display:none;">
                            <div class="enroll-section-title">👥 报名人数</div>
                            <div id="activity-fee-display"></div>
                            <div class="activity-count-form" id="activity-count-form">
                                <div class="enroll-activity-fee-row" id="activity-count-adult-row" style="display:none;">
                                    <div class="fee-who">成人<small>陪同</small></div>
                                    <span class="fee-tag" id="activity-adult-fee-label"></span>
                                    ...
                                    <span class="fee-sub" id="activity-adult-subtotal">= ¥0</span>
                                </div>
                                <div class="enroll-activity-fee-row" id="activity-count-student-row" style="display:none;">
                                    <div class="fee-who">学员<small>报名</small></div>
                                    <span class="fee-tag" id="activity-student-fee-label"></span>
                                    <input type="number" id="activity-student-count" class="activity-count-input" min="1" value="1" oninput="recalcActivityTotal()">
                                    <span class="fee-unit">人</span>
                                    <span class="fee-sub" id="activity-student-subtotal">= ¥0</span>
                                </div>
                            </div>
                            <div class="enroll-activity-hint enroll-activity-hint-warn" id="activity-deduct-hint" style="display:none;">
                                <span>⏳</span><span id="activity-deduct-text"></span>
                            </div>
                            <div class="enroll-activity-hint enroll-activity-hint-err" id="activity-capacity-hint" style="display:none;"></div>
                            <div class="enroll-activity-total-bar" id="activity-total-bar">
                                <span class="tl">报名费用合计</span>
                                <span class="ta"><small>¥</small><span id="activity-total-amount">0.00</span></span>
                            </div>
                        </div>

                        <!-- 剩余课时 & 考勤学科提示 — 选活动后展示 -->
                        <div class="enroll-section" id="enroll-activity-step-subject" style="display:none;">
                            <div class="enroll-section-title">📊 请选择活动考勤时优先扣课时的学科</div>
                            <div id="activity-subject-hours" style="margin-bottom:10px;">
                                <div style="text-align:center;color:#999;padding:8px;">加载中...</div>
                            </div>
                            <div style="font-size:11px;color:var(--color-text-muted);margin-bottom:6px;">勾选学科行即可指定优先扣减对象；不勾选则考勤时手动选择</div>
                        </div>

                        <!-- 课程顾问 — 选活动后展示 -->
                        <div id="enroll-activity-advisor-section" style="display:none;">
                            <div class="enroll-section-title">👤 课程顾问</div>
                            <div class="searchable-dropdown" id="dropdown-activity-advisor">
                                <input type="text" class="searchable-input" placeholder="输入姓名搜索课程顾问..." autocomplete="off" data-placeholder="输入姓名搜索课程顾问...">
                                <input type="hidden" class="searchable-value" value="0">
                                <div class="searchable-menu"></div>
                            </div>
                        </div>
                        ...
                        <!-- 支付方式 — 选活动后展示 -->
                        <div id="enroll-activity-pay-section" style="display:none;">
                            ...
                            <div class="enroll-payment-row">
                                <div class="enroll-payment-card">
                                    <div class="enroll-payment-header">
                                        <span class="enroll-payment-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><line x1="12" y1="10" x2="12" y2="14"/></svg></span>
                                        <span class="enroll-payment-label">现金</span>
                                    </div>
                                    <input type="number" id="activity-pay-cash" class="enroll-payment-input" step="0.01" min="0" value="0" oninput="checkActivityPayMatch()" placeholder="0.00">
                                </div>
                                <div class="enroll-payment-card">
                                    <div class="enroll-payment-header">
                                        <span class="enroll-payment-icon enroll-payment-icon-meituan"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg></span>
                                        <span class="enroll-payment-label">美团</span>
                                    </div>
                                    <input type="number" id="activity-pay-meituan" class="enroll-payment-input" step="0.01" min="0" value="0" oninput="checkActivityPayMatch()" placeholder="0.00">
                                </div>
                                <div class="enroll-payment-card enroll-payment-card-balance">
                                    <div class="enroll-payment-header">
                                        <span class="enroll-payment-icon enroll-payment-icon-balance"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg></span>
                                    ...
                                    <input type="number" id="activity-pay-balance" class="enroll-payment-input" step="0.01" min="0" value="0" oninput="checkActivityPayMatch()" placeholder="0.00">
                                </div>
                                <div class="enroll-payment-card">
                                    <div class="enroll-payment-header">
                                        <span class="enroll-payment-icon enroll-payment-icon-online"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg></span>
                                        <span class="enroll-payment-label">线上支付</span>
                                    </div>
                                    <input type="number" id="activity-pay-online" class="enroll-payment-input" step="0.01" min="0" value="0" oninput="checkActivityPayMatch()" placeholder="0.00">
                                </div>
                                <div class="enroll-payment-card">
                                    <div class="enroll-payment-header">
                                        <span class="enroll-payment-icon enroll-payment-icon-tonglian"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18"/><path d="M9 21V9"/></svg></span>
                                        <span class="enroll-payment-label">通联二维码</span>
                                    </div>
                                    <input type="number" id="activity-pay-tonglian" class="enroll-payment-input" step="0.01" min="0" value="0" oninput="checkActivityPayMatch()" placeholder="0.00">
                                </div>
                                <div class="enroll-payment-card">
                                    <div class="enroll-payment-header">
                                        <span class="enroll-payment-icon enroll-payment-icon-zhishouyin"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="5"/><line x1="8" y1="12" x2="16" y2="12"/></svg></span>
                                        <span class="enroll-payment-label">智收银</span>
                                    </div>
                                    <input type="number" id="activity-pay-zhishouyin" class="enroll-payment-input" step="0.01" min="0" value="0" oninput="checkActivityPayMatch()" placeholder="0.00">
                                </div>
                                <div class="enroll-payment-card">
                                    <div class="enroll-payment-header">
                                        <span class="enroll-payment-icon enroll-payment-icon-douyin"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"/></svg></span>
                                        <span class="enroll-payment-label">抖音</span>
                                    </div>
                                    <input type="number" id="activity-pay-douyin" class="enroll-payment-input" step="0.01" min="0" value="0" oninput="checkActivityPayMatch()" placeholder="0.00">
                                </div>
                            </div>
                            <div id="activity-pay-hint" class="enroll-payment-hint" style="display:none;"></div>
                        </div>

                        <!-- 确认按钮栏 -->
                        <div class="enroll-activity-confirm-bar" id="enroll-activity-confirm-bar" style="display:none;">
                            <button class="btn btn-outline btn-lg" onclick="resetActivityEnroll()">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="1 4 1 10 7 10"/><path d="M1 10a9 9 0 0 1 13.7-1.3"/></svg>
                                重新选择
                            </button>
                            <button class="btn btn-capsule btn-activity-confirm" onclick="confirmActivityEnroll()" id="btn-activity-confirm">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>
                                确认支付
                            </button>
                        </div>
            ...
            <section class="content-panel" id="panel-work-records">
                ...
                <div class="section-tab-content">
                    ...
                    <!-- 转课记录 tab -->
                    <div class="sec-panel" id="tab-course-transfer-records">
                        <div class="panel-toolbar" id="toolbar-course-transfer-records">
                            <div class="filter-bar">
                                <select id="filter-ctr-campus"><option value="">全部校区</option></select>
                                <input type="text" id="filter-ctr-student" placeholder="学员姓名/手机号">
                                <input type="date" id="filter-ctr-date-from">
                                <input type="date" id="filter-ctr-date-to">
                                <select id="filter-ctr-status">
                                    <option value="">全部状态</option>
                                    <option value="正常">正常</option>
                                    <option value="已撤销">已撤销</option>
                                </select>
                                <button class="btn btn-primary btn-sm" onclick="loadCourseTransferRecords()">查询</button>
                            </div>
                        </div>
                        <div class="table-wrap">
                            <table id="table-course-transfer-records">
                                <thead>
                                    <tr>
                                        <th>学员</th><th>手机号</th><th>学号</th><th>校区</th>
                                        <th>原课程</th><th>转出课时</th><th>转出价值</th>
                                        <th>目标课程</th><th>转入课时</th><th>转入价值</th>
                                        <th>跨科</th><th>转课时间</th><th>状态</th><th>操作</th>
                                    </tr>
                                </thead>
                                <tbody></tbody>
                            </table>
                        </div>
                        <div id="pagination-course-transfer-records" class="pagination"></div>
                    </div>
                    <!-- 转校记录 tab -->
                ...
                <div class="sec-panel" id="tab-ta-sales">
                    <div class="toolbar">
                        <div class="ta-filter-bar">
                            ...
                            <!-- 搜索按钮 -->
                            <!-- 日期范围：开始 -->
                            <div class="ta-filter-group ta-filter-date-from">
                                <span class="ta-filter-icon">
                                ...
                                <input type="date" id="ta-sales-date-from" class="ta-filter-input" placeholder="开始日期">
                            </div>
                            <span class="ta-filter-date-sep">至</span>
                            <!-- 日期范围：结束 -->
                            <div class="ta-filter-group ta-filter-date-to">
                                <span class="ta-filter-icon">
                                    <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                                </span>
                                <input type="date" id="ta-sales-date-to" class="ta-filter-input" placeholder="结束日期">
                            </div>
            ...
            <div class="price-right price-detail">
                ...
                <div class="price-detail-table-wrap">
                    <table id="price-item-table" class="price-item-table">
                        <thead><tr><th>报价单名称</th><th>课时数量</th><th>课时价格</th><th>优惠方案</th><th>课时优惠券</th><th>教材包</th><th>教材包原价</th><th>商品券</th><th>赠送课时</th><th>实际价格</th><th width="120">操作</th></tr></thead>
    ...
    <div class="modal-overlay" id="modal-classroom-form">
        <div class="modal modal-classroom">
            <div class="modal-header">
                <h3 id="modal-classroom-title">新增教室</h3>
                <button class="modal-close" onclick="closeModal('modal-classroom-form')" aria-label="关闭">&times;</button>
            </div>
            <div class="modal-body">
                <input type="hidden" id="edit-classroom-id">
                <div class="classroom-form-group">
                    <label for="classroom-name">教室名称 <span class="required">*</span></label>
                    <input type="text" id="classroom-name" maxlength="50" placeholder="请输入教室名称" autocomplete="off">
                </div>
                <div class="classroom-form-row">
                    <div class="classroom-form-group">
                        <label for="classroom-campus">所属校区</label>
                        <select id="classroom-campus"><option value="">请选择校区</option></select>
                    </div>
                    <div class="classroom-form-group">
                        <label for="classroom-capacity">容纳人数</label>
                        <input type="number" id="classroom-capacity" min="0" placeholder="0">
                    </div>
                </div>
                <div class="classroom-form-group">
                    <label for="classroom-remark">备注</label>
                    <textarea id="classroom-remark" rows="4" placeholder="备注信息（选填）" maxlength="200"></textarea>
                    <span class="classroom-char-count" id="classroom-char-count">0 / 200</span>
                </div>
            </div>
            <div class="modal-footer">
                <button class="btn btn-cancel" onclick="closeModal('modal-classroom-form')">取消</button>
                <button class="btn btn-primary" onclick="saveClassroom()" id="classroom-save-btn">保存</button>
            </div>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="modal-refund-apply">
        <div class="modal modal-lg">
            ...
            <div class="modal-body">
                ...
                <input type="hidden" id="refund-apply-transfer-id">
                <input type="hidden" id="refund-apply-ctr-id">
                ...
                <div class="dp-card">
                    ...
                    <div class="dp-card-body">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group" style="flex:1;">
                                <label>适用方案类型</label>
                                <select id="coupon-plan-type" class="form-input">
                                    <option value="">不限</option>
                                    <option value="新报">新报</option>
                                    <option value="续费">续费</option>
                                    <option value="小课包">小课包</option>
                                </select>
                            </div>
                            <div class="form-group" style="flex:1;">
                                <label>优惠金额 (元)</label>
        ...
        <div class="modal modal-lg" style="width:1100px;max-width:95vw;">
            ...
            <div class="modal-body" style="padding:20px 24px;">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <select id="att-activity-teacher" style="width:200px;height:36px;border:1px solid #ddd;border-radius:8px;padding:0 8px;">
                        <option value="">请选择</option>
                    </select>
                </div>
