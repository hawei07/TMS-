...
// ==================== 活动管理 ====================
let activityOrgFlat = []; // 缓存的完整组织扁平列表 [{id, name, type, parent_id}]
let activityCampusTree = []; // 由扁平列表构建的可选校区树
let activityCampusState = {}; // campusId -> {checked, capacity}
let activityCampusExpanded = {}; // groupId -> 是否展开（默认展开）

async function loadActivityCampusData() {
    if (activityOrgFlat.length) return activityOrgFlat;
    try {
        const res = await fetch(API_BASE + 'list_organizations');
        const data = await res.json();
        const flat = (data && data.data && data.data.flat) ? data.data.flat : [];
        activityOrgFlat = flat;
        return activityOrgFlat;
    } catch (e) {
        return [];
    }
}
...
function buildCampusTree(flat) {
    ...
    // 仅保留「校区」或包含「校区」后代的节点
    function hasCampusDesc(n) { return n.type === '校区' || n.children.some(hasCampusDesc); }
    function prune(nodes) {
        const out = [];
        nodes.forEach(n => { if (hasCampusDesc(n)) out.push({ ...n, children: prune(n.children) }); });
        return out;
    }
    return prune(roots);
}

function collectCampusLeaves(node) {
    if (node.type === '校区') return [node];
    return node.children.reduce((arr, c) => arr.concat(collectCampusLeaves(c)), []);
}
function countCampusLeaves(node) {
    if (node.type === '校区') return 1;
    return node.children.reduce((s, c) => s + countCampusLeaves(c), 0);
}
function countCampusChecked(node) {
    if (node.type === '校区') return (activityCampusState[node.id] && activityCampusState[node.id].checked) ? 1 : 0;
    return node.children.reduce((s, c) => s + countCampusChecked(c), 0);
}
function findCampusNode(id, nodes) {
    nodes = nodes || activityCampusTree;
    for (const n of nodes) {
        if (n.id == id) return n;
        if (n.children) { const r = findCampusNode(id, n.children); if (r) return r; }
    }
    return null;
}

function renderCampusNode(node, depth, forceExpand) {
    const pad = 10 + depth * 22;
    if (node.children && node.children.length) {
        const checked = countCampusChecked(node);
        const total = countCampusLeaves(node);
        const expanded = forceExpand !== undefined ? forceExpand : (activityCampusExpanded[node.id] !== false);
        return '<div class="campus-tree-node group" data-id="' + node.id + '" style="padding-left:' + pad + 'px;">' +
            '<span class="ct-caret ' + (expanded ? '' : 'collapsed') + '" onclick="toggleCampusCaret(' + node.id + ')">▾</span>' +
            '<input type="checkbox" class="campus-group-check" ' + (checked ? 'checked' : '') + ' onchange="toggleCampusGroup(' + node.id + ', this.checked)">' +
            '<span class="ct-name">' + esc(node.name) + '<span class="ct-tag">' + (node.type === '部门' ? '区域' : '') + '</span></span>' +
            '<span class="ct-count">' + checked + '/' + total + ' 校区</span>' +
            '</div>' +
            '<div class="campus-tree-children" ' + (expanded ? '' : 'style="display:none"') + '>' +
            node.children.map(c => renderCampusNode(c, depth + 1, forceExpand)).join('') +
            '</div>';
    }
    const st = activityCampusState[node.id] || { checked: false, capacity: '' };
    return '<div class="campus-tree-node campus" data-id="' + node.id + '" style="padding-left:' + pad + 'px;">' +
        '<span class="ct-caret leaf">▾</span>' +
        '<input type="checkbox" class="campus-leaf-check" ' + (st.checked ? 'checked' : '') + ' onchange="toggleCampusLeaf(' + node.id + ', this.checked)">' +
        '<span class="ct-name">' + esc(node.name) + '</span>' +
        '<span class="ct-cap"><span>报名人数</span>' +
        '<input type="number" min="0" class="campus-capacity-input" value="' + st.capacity + '" placeholder="0" ' + (st.checked ? '' : 'disabled') + ' oninput="onCampusCapacity(' + node.id + ', this.value)"> 人</span>' +
        '</div>';
}

function rerenderCampusTree() {
    const el = document.getElementById('activity-campus-tree');
    if (!el) return;
    el.innerHTML = activityCampusTree.map(n => renderCampusNode(n, 0)).join('');
    setCampusIndeterminate();
    updateCampusSummary();
    updateActivitySubmitState();
}
function setCampusIndeterminate() {
    document.querySelectorAll('#activity-campus-tree .campus-tree-node.group').forEach(node => {
        const cb = node.querySelector('.campus-group-check');
        const g = findCampusNode(parseInt(node.dataset.id));
        if (!g) return;
        const t = countCampusLeaves(g), c = countCampusChecked(g);
        cb.indeterminate = c > 0 && c < t;
    });
}
function toggleCampusCaret(id) {
    activityCampusExpanded[id] = activityCampusExpanded[id] === false;
    rerenderCampusTree();
}
function toggleCampusGroup(id, checked) {
    const node = findCampusNode(id);
    if (!node) return;
    collectCampusLeaves(node).forEach(l => { activityCampusState[l.id].checked = checked; });
    rerenderCampusTree();
}
function toggleCampusLeaf(id, checked) {
    if (activityCampusState[id]) activityCampusState[id].checked = checked;
    rerenderCampusTree();
}
function onCampusCapacity(id, val) {
    if (activityCampusState[id]) activityCampusState[id].capacity = val;
    updateCampusSummary();
}
function updateCampusSummary() {
    const el = document.getElementById('campus-summary');
    if (!el) return;
    const ids = Object.keys(activityCampusState);
    const checked = ids.filter(id => activityCampusState[id].checked);
    const total = checked.reduce((s, id) => s + (parseInt(activityCampusState[id].capacity) || 0), 0);
    el.innerHTML = '已选 <b>' + checked.length + '</b> 个校区 · 合计 <b>' + total + '</b> 人';
}

function filterActivityCampusTree(q) {
    q = (q || '').trim().toLowerCase();
    const el = document.getElementById('activity-campus-tree');
    if (!el) return;
    if (!q) { rerenderCampusTree(); return; }
    function matchFilter(nodes) {
        const out = [];
        nodes.forEach(n => {
            if (n.children && n.children.length) {
                const kids = matchFilter(n.children);
    ...
    const filtered = matchFilter(activityCampusTree);
    el.innerHTML = filtered.map(n => renderCampusNode(n, 0, true)).join('');
    setCampusIndeterminate();
    const empty = document.getElementById('campus-empty-msg');
...
function initActivityCampusUI() {
    ...
    if (selectAllBtn) {
        selectAllBtn.onclick = function() {
            Object.keys(activityCampusState).forEach(id => { activityCampusState[id].checked = true; });
            rerenderCampusTree();
        };
    }
    if (deselectAllBtn) {
        deselectAllBtn.onclick = function() {
            Object.keys(activityCampusState).forEach(id => { activityCampusState[id].checked = false; });
            rerenderCampusTree();
        };
    }
    if (applyBatchBtn) {
        applyBatchBtn.onclick = function() {
            const v = batchCapInput ? batchCapInput.value : '';
            if (v === '') return;
            Object.keys(activityCampusState).forEach(id => {
                if (activityCampusState[id].checked) activityCampusState[id].capacity = v;
            });
            rerenderCampusTree();
        };
    }
...
function updateActivitySubmitState() {
    ...
    const hasCampus = Object.keys(activityCampusState).some(id => activityCampusState[id] && activityCampusState[id].checked);
...
async function addActivityDeductRow(type, subjectId, subjectName, deductLessons) {
    ...
    // 扣课单位：课时，且仅允许偶数（>=2）
    const normLessons = deductLessons
        ? (deductLessons % 2 === 0 ? deductLessons : Math.max(2, (Math.floor(deductLessons / 2) + 1) * 2))
        : '';
    ...
    row.innerHTML = '<select class="deduct-subject" data-type="' + type + '">' + optionsHtml + '</select>' +
        '<span class="deduct-label">扣课时数：</span>' +
        '<input type="number" class="deduct-lessons" min="2" step="2" value="' + normLessons + '" placeholder="2">' +
...
function renderActivityCampusRows(campuses) {
    const treeEl = document.getElementById('activity-campus-tree');
    if (!treeEl) return;
    if (!activityOrgFlat.length) {
        treeEl.innerHTML = '<span style="color:#999;font-size:13px;">正在加载校区...</span>';
    ...
    const selMap = {};
    (campuses || []).forEach(c => {
        const cid = c.campus_id ? parseInt(c.campus_id) : 0;
        if (cid) selMap[cid] = c.max_capacity || 0;
    });
    activityCampusState = {};
    activityOrgFlat.filter(n => n.type === '校区').forEach(n => {
        activityCampusState[n.id] = { checked: !!selMap[n.id], capacity: selMap[n.id] || '' };
    });
    activityCampusTree = buildCampusTree(activityOrgFlat);
    rerenderCampusTree();
}

function addActivityCampusRow() {
    // 校区树已在 renderActivityCampusRows 中完整渲染
}
...
async function saveActivity() {
    ...
    const campuses = [];
    Object.keys(activityCampusState).forEach(id => {
        const st = activityCampusState[id];
        if (st && st.checked) {
            campuses.push({
                campus_id: parseInt(id),
                max_capacity: parseInt(st.capacity) || 0
            });
        }
    });
    ...
    ['adult', 'student'].forEach(type => {
        ...
        rows.forEach(row => {
            ...
            if (subId && lessons > 0) {
                if (lessons < 2 || lessons % 2 !== 0) {
                    const subName = select.options[select.selectedIndex] ? select.options[select.selectedIndex].text : '该学科';
                    showToast('扣课时数必须为不小于 2 的偶数（' + subName + '）', 'error');
                    deductValid = false;
                    return;
                }
...
async function preloadInlineDiscountData() {
    ...
    try {
        ...
        priceItemCourseCoupons = courseCouponRes.data || [];
        priceItemCoupons = productCouponRes.data || [];
...
async function savePlan() {
    ...
    if (planId > 0) {
        ...
        const items = (existingPlan && existingPlan.items) ? existingPlan.items.map((item, idx) => ({
            ...
            actual_price: (() => {
                const isSmall = (existingPlan.plan_type === '小课包');
                if (isSmall) return parseFloat(item.actual_price) || 0;
                let d = 0, taPrice = 0;
                const dpId = parseInt(item.discount_plan_id) || 0, cId = parseInt(item.coupon_id) || 0;
                const taId = parseInt(item.teaching_aid_id) || 0, pcId = parseInt(item.product_coupon_id) || 0;
...
function recalcItemActualPrice() {
    ...
    const courseCouponSelect = document.getElementById('price-item-course-coupon');
    const couponSelect = document.getElementById('price-item-coupon');
    ...
    if (courseCouponSelect && courseCouponSelect.value) {
...
// 加载课时优惠券下拉
async function loadPriceItemCourseCouponOptions(selectedValue) {
...
<button class="btn btn-danger btn-sm" onclick="showRefundApplyModal(${r.order_id})" style="font-size:11px;padding:2px 8px;">退费</button>`;
        ...
        // 剩余课时 = 0 时添加 row-depleted
        const remaining = parseInt(r.remaining_lessons) || 0;
        const lcForDepleted = parseInt(r.lesson_count) || 0;
        const rowClass = (remaining === 0 && lcForDepleted > 0 && refundStatus === '正常') ? ' class="row-depleted"' : '';
        // 转出课时显示
...
function preselectEnrollCampusFromStudentData(studentData) {
    const campusSel = document.getElementById('enroll-campus-select');
    if (!campusSel || !studentData) return null;

    const sstRecords = studentData.sst_records || [];
    let primaryCampus = '';
    if (sstRecords.length > 0) primaryCampus = sstRecords[0].campus_name || '';
    if (!primaryCampus && studentData.primary_campus) primaryCampus = studentData.primary_campus;

    let campusMatched = false;
    if (primaryCampus) {
        const candidates = primaryCampus.split(/[,，、]\s*/).filter(Boolean);
        for (let ci = 0; ci < candidates.length && !campusMatched; ci++) {
            for (let oi = 1; oi < campusSel.options.length; oi++) {
                const optText = campusSel.options[oi].text.replace(/\s+/g, '');
                const candText = candidates[ci].replace(/\s+/g, '');
                if (optText === candText) {
                    campusSel.selectedIndex = oi;
                    campusMatched = true;
                    break;
                }
            }
    ...
    if (!campusMatched && sstRecords.length > 0 && sstRecords[0].campus_id) {
        const sstCampusId = String(sstRecords[0].campus_id);
        for (let oi = 1; oi < campusSel.options.length; oi++) {
            if (campusSel.options[oi].value === sstCampusId) {
                campusSel.selectedIndex = oi;
                campusMatched = true;
                break;
            }
    ...
    if (!campusMatched && studentData.primary_campus_ids) {
        const pcIds = studentData.primary_campus_ids.split(',').map(function(s) { return s.trim(); }).filter(Boolean);
        for (let ci = 0; ci < pcIds.length && !campusMatched; ci++) {
            for (let oi = 1; oi < campusSel.options.length; oi++) {
                if (campusSel.options[oi].value === pcIds[ci]) {
                    campusSel.selectedIndex = oi;
                    campusMatched = true;
                    break;
                }
    ...
    if (!campusMatched) return null;
    currentEnrollCampusId = parseInt(campusSel.value) || null;
    currentEnrollCampusName = campusSel.options[campusSel.selectedIndex]?.text || '';
    return {
        id: currentEnrollCampusId,
        name: currentEnrollCampusName,
        displayName: primaryCampus || currentEnrollCampusName
    };
}
...
async function goEnroll(studentId) {
    ...
    try {
        ...
        // determine primary campus from SST records, fallback to orders
        var sstRecords = data.sst_records || [];
        var primaryCampus = '';
        if (sstRecords.length > 0) { primaryCampus = sstRecords[0].campus_name || ''; }
        if (!primaryCampus && data.primary_campus) { primaryCampus = data.primary_campus; }
        var primaryCampusIds = data.primary_campus_ids || '';
    ...
    let preSelectedCampusId = null;
    if (primaryCampus) {
        const campusSel = document.getElementById('enroll-campus-select');
        let campusMatched = false;

        // 策略1：文本精确匹配（兼容旧逻辑，normalize去除所有空白）
        const candidates = primaryCampus.split(/[,，、]\s*/).filter(Boolean);
        for (let ci = 0; ci < candidates.length && !campusMatched; ci++) {
            for (let oi = 1; oi < campusSel.options.length; oi++) {
                const optText = campusSel.options[oi].text.replace(/\s+/g, '');
        ...
        // 策略2：ID精确匹配（从 sst_records 获取 campus_id）
        if (!campusMatched && sstRecords.length > 0 && sstRecords[0].campus_id) {
            const sstCampusId = String(sstRecords[0].campus_id);
            for (let oi = 1; oi < campusSel.options.length; oi++) {
                if (campusSel.options[oi].value === sstCampusId) {
        ...
        // 策略3：ID精确匹配（从 primary_campus_ids 兜底）
        if (!campusMatched && primaryCampusIds) {
            const pcIds = primaryCampusIds.split(',').map(function(s) { return s.trim(); }).filter(Boolean);
            for (let ci = 0; ci < pcIds.length && !campusMatched; ci++) {
                for (let oi = 1; oi < campusSel.options.length; oi++) {
                    if (campusSel.options[oi].value === pcIds[ci]) {
                        campusSel.selectedIndex = oi;
        ...
        if (campusMatched) {
            currentEnrollCampusId = parseInt(campusSel.value);
            currentEnrollCampusName = campusSel.options[campusSel.selectedIndex].text;
            preSelectedCampusId = currentEnrollCampusId;
        } else {
            currentEnrollCampusName = '';
        }
    }

    // 校区预选成功 → 自动显示类型选择 + 加载课程 + 进入课程报名流程
    if (preSelectedCampusId) {
        document.getElementById('enroll-type-select').style.display = '';
        await loadEnrollCoursePicker(preSelectedCampusId);
    ...
    document.getElementById('ecf-sticky-bar').style.display = 'none';
...
async function goEnrollFromResource(resourceId) {
    ...
    // 如果资源已有对应学员，自动加载学员完整信息并预选校区
    let linkedStudentId = null;
    if (resource) {
        try {
            let matched = null;
            const linkedRes = await fetch(API_BASE + 'list_students&page=1&page_size=1&resource_id=' + encodeURIComponent(resourceId));
            const linkedData = await linkedRes.json();
            matched = (linkedData.data || [])[0] || null;
            if (!matched && resource.phone) {
                const sRes = await fetch(API_BASE + 'list_students&page=1&page_size=5&keyword=' + encodeURIComponent(resource.phone));
                const sData = await sRes.json();
                matched = (sData.data || []).find(s => s.phone === resource.phone) || null;
            }
            if (matched) {
                linkedStudentId = matched.id;
                currentEnrollStudentId = matched.id;
                // 加载学员详情（学号、校区、余额、剩余课时）
                const dRes = await fetch(API_BASE + 'get_student&id=' + matched.id);
                const dData = await dRes.json();
                if (dData && dData.student) {
                    document.getElementById('enroll-info-no').textContent = dData.student.student_no || '-';
                    document.getElementById('enroll-banner-avatar').textContent = (dData.student.name || '?').charAt(0);
                    const sstRecords = dData.sst_records || [];
                    let primaryCampus = '';
                    if (sstRecords.length > 0) { primaryCampus = sstRecords[0].campus_name || ''; }
                    if (!primaryCampus && dData.primary_campus) { primaryCampus = dData.primary_campus; }
                    document.getElementById('enroll-info-campus').textContent = primaryCampus || '未设置';
                    const campusMatch = preselectEnrollCampusFromStudentData(dData);
                    if (campusMatch && campusMatch.id) {
                        document.getElementById('enroll-type-select').style.display = '';
                        await loadEnrollCoursePicker(campusMatch.id);
                        selectEnrollType('course');
                        initEnrollReferralDropdowns();
                    }
                    // 预选校区下拉
                    if (!currentEnrollCampusId && primaryCampus) {
                        const campusSel = document.getElementById('enroll-campus-select');
                        let campusMatched = false;
                        ...
                        for (let ci = 0; ci < candidates.length && !campusMatched; ci++) {
                            for (let oi = 1; oi < campusSel.options.length; oi++) {
                                // normalize: 去除所有空白字符后再比较，兼容全角/半角空格差异
                                const optText = campusSel.options[oi].text.replace(/\s+/g, '');
                                const candText = candidates[ci].replace(/\s+/g, '');
                                if (optText === candText) {
                                    campusSel.selectedIndex = oi;
                        ...
                        // 策略3：ID精确匹配（从 primary_campus_ids 兜底）
                        if (!campusMatched && dData.primary_campus_ids) {
                            const pcIds = dData.primary_campus_ids.split(',').map(function(s) { return s.trim(); }).filter(Boolean);
                            for (let ci = 0; ci < pcIds.length && !campusMatched; ci++) {
                                for (let oi = 1; oi < campusSel.options.length; oi++) {
                                    if (campusSel.options[oi].value === pcIds[ci]) {
                                        campusSel.selectedIndex = oi;
                        ...
                        if (campusMatched) {
                            ...
                            // 加载该校区下的课程并显示课程流
                            document.getElementById('enroll-type-select').style.display = '';
                            await loadEnrollCoursePicker(currentEnrollCampusId);
                            selectEnrollType('course');
                            initEnrollReferralDropdowns();
                        }
                    }
                    // 账户余额 + 剩余课时
                    try {
                        const acctRes = await fetch(API_BASE + 'get_student_account&student_id=' + matched.id);
                        const acctData = await acctRes.json();
                        const bal = parseFloat(acctData.balance) || 0;
                        document.getElementById('enroll-balance-avail').textContent = '(¥' + bal.toLocaleString('zh-CN', {minimumFractionDigits: 2}) + ')';
                        document.getElementById('enroll-payment-balance').max = bal;
                        const balVal = document.getElementById('esb-balance').querySelector('.esb-chip-val');
                        if (balVal) balVal.textContent = '¥' + bal.toLocaleString('zh-CN', {minimumFractionDigits: 2});
                        if (bal <= 0) document.getElementById('esb-balance').classList.add('zero');
                        else document.getElementById('esb-balance').classList.remove('zero');
                    } catch (e) { /* ignore */ }
                    const summary = dData.summary || {};
                    const remaining = Math.max(0, (parseInt(summary.total_lessons) || 0) - (parseInt(summary.consumed_lessons) || 0));
                    const hoursEl = document.getElementById('esb-hours').querySelector('.esb-chip-val');
                    if (hoursEl) hoursEl.textContent = remaining;
                }
            }
        } catch (e) { /* ignore */ }
    ...
    if (currentEnrollCampusId) {
        document.getElementById('enroll-type-select').style.display = '';
        if (!currentEnrollType) {
            await loadEnrollCoursePicker(currentEnrollCampusId);
            selectEnrollType('course');
        }
    } else {
    ...
    if (!currentEnrollCampusId) currentEnrollType = null;
...
function renderEnrollPlansList(plans) {
    const tbody = document.getElementById('enroll-plans-list');
    if (!plans || plans.length === 0) {
        tbody.innerHTML = '';
        document.getElementById('ecf-plan-empty').style.display = 'block';
        document.getElementById('ecf-plan-table').style.display = 'none';
        return;
    }
    document.getElementById('ecf-plan-empty').style.display = 'none';
    document.getElementById('ecf-plan-table').style.display = '';
    let html = '';
    for (var i = 0; i < plans.length; i++) {
        ...
        var tc = '';
        var pt2 = p.plan_type || '';
        if (pt2 === '新报') { typeTag = '新报'; tc = 'ecf-tag ecf-tag-new'; }
        else if (pt2 === '续费') { typeTag = '续费'; tc = 'ecf-tag ecf-tag-renew'; }
        else if (pt2 === '小课包') { typeTag = '小课包'; tc = 'ecf-tag ecf-tag-pack'; }
        var total = (p.items || []).reduce(function(s, it) { return s + (parseFloat(it.actual_price) || 0); }, 0);
        var totalLessons = (p.items || []).reduce(function(s, it) { return s + (parseInt(it.lesson_count) || 0); }, 0);
        var itemCount = (p.items || []).length;
        var sel = currentEnrollPlanId === p.id;
        html += '<tr class="' + (sel ? 'sel' : '') + '" data-plan-id="' + p.id + '" onclick="selectEnrollPlan(' + p.id + ')">' +
            '<td class="col-radio">' + (sel ? '●' : '○') + '</td>' +
            '<td class="col-name" style="font-weight:600">' + esc(p.name) + '</td>' +
            '<td class="col-type">' + (typeTag ? '<span class="' + tc + '">' + typeTag + '</span>' : '') + '</td>' +
            '<td class="col-r" style="color:#6b6580">' + itemCount + ' 项</td>' +
            '<td class="col-r" style="color:#6b6580">' + totalLessons + '</td>' +
            '<td class="col-r" style="font-weight:700;color:#7C3AED">¥' + total.toFixed(2) + '</td>' +
        '</tr>';
    }
...
function selectEnrollPlan(planId) {
    ...
    // 高亮选中行
    document.querySelectorAll('#enroll-plans-list tr').forEach(function(tr) { tr.classList.remove('sel'); });
    var planRow = document.querySelector('#enroll-plans-list tr[data-plan-id="' + planId + '"]');
    if (planRow) planRow.classList.add('sel');
    ...
    let total = 0;
    tbody.innerHTML = plan.items.map(function(item) {
        var dpHtml = item.discount_plan_name ? ('<span class="ecf-tag ecf-tag-dc">' + esc(item.discount_plan_name) + (item.discount_plan_amount ? ' ¥' + Number(item.discount_plan_amount).toFixed(2) : '') + '</span>') : '-';
        var cpHtml = item.coupon_name ? ('<span class="ecf-tag ecf-tag-cp">' + esc(item.coupon_name) + (item.coupon_amount ? ' ¥' + Number(item.coupon_amount).toFixed(2) : '') + '</span>') : '-';
        var aidHtml = item.teaching_aid_name ? esc(item.teaching_aid_name) : '-';
        var aidPriceHtml = item.teaching_aid_price ? '¥' + Number(item.teaching_aid_price).toFixed(2) : '-';
        var pcHtml = item.product_coupon_name ? ('<span class="ecf-tag ecf-tag-aid">' + esc(item.product_coupon_name) + (item.product_coupon_amount ? ' ¥' + Number(item.product_coupon_amount).toFixed(2) : '') + '</span>') : '-';
        total += parseFloat(item.actual_price) || 0;
        return '<tr style="cursor:default">' +
            '<td style="font-weight:600">' + esc(item.name) + '</td>' +
            '<td class="col-r">' + (item.lesson_count || 0) + '</td>' +
            '<td class="col-r" style="color:#9895A8">¥' + Number(item.unit_price).toFixed(2) + '</td>' +
            '<td>' + dpHtml + '</td>' +
            '<td>' + cpHtml + '</td>' +
            '<td>' + aidHtml + '</td>' +
            '<td class="col-r" style="color:#9895A8">' + aidPriceHtml + '</td>' +
            '<td>' + pcHtml + '</td>' +
            '<td class="col-r">' + (item.gifted_lessons || 0) + '</td>' +
            '<td class="col-r" style="font-weight:700">¥' + Number(item.actual_price).toFixed(2) + '</td>' +
        '</tr>';
    }).join('');
    ...
    // 更新底部固定栏
    var courseName = currentEnrollCourses.find(function(c) { return c.id == currentEnrollCourseId; });
    document.getElementById('ecf-sticky-summary').innerHTML =
        '<span class="ss-course">' + (courseName ? esc(courseName.name) : '') + '</span>' +
        '<span class="ss-sep">·</span>' +
        '<span class="ss-plan">' + esc(plan.name) + '（' + ((plan.items || []).reduce(function(s, i) { return s + (parseInt(i.lesson_count) || 0); }, 0)) + '课时）</span>';
    document.getElementById('ecf-sticky-total-val').textContent = total.toFixed(2);
    document.getElementById('ecf-sticky-bar').style.display = 'block';
}
...
function updatePaymentHint() {
    ...
    if (Math.abs(diff) < 0.01) {
        ...
        hint.className = 'ecf-pay-status ok';
        const parts = [];
        ...
        hint.style.display = 'block';
        hint.className = 'ecf-pay-status warn';
...
function updateEcfPayRows() {
    var ids = ['cash','meituan','balance','online','tonglian','zhishouyin','douyin'];
    for (var i = 0; i < ids.length; i++) {
        var row = document.getElementById('ecf-pay-' + ids[i] + '-row');
        var v = parseFloat(document.getElementById('enroll-payment-' + ids[i]).value) || 0;
        if (row) { if (v > 0) row.classList.add('has-val'); else row.classList.remove('has-val'); }
    }
}

function toggleEcfRefMore() {
    var toggle = document.getElementById('ecf-ref-toggle');
    var more = document.getElementById('ecf-ref-more');
...
async function loadEnrollCoursePicker(campusId) {
    ...
    try {
        ...
        renderCoursePickerRows(enrollCoursePickerData);
    } catch (e) {
        pickerSearchEl.disabled = true;
...
function renderCoursePickerChips(courses) {
    ...
    let html = '<span class="ecf-chip on" data-subject="">全部（' + courses.length + '）</span>';
    subjects.forEach(s => {
        html += '<span class="ecf-chip" data-subject="' + esc(s) + '">' + esc(s) + '<span class="cnt">' + subjectMap[s] + '</span></span>';
    });
    pickerChipsEl.innerHTML = html;

    pickerChipsEl.querySelectorAll('.ecf-chip').forEach(chip => {
...
function applyCoursePickerFilters() {
    ...
    const activeChip = pickerChipsEl.querySelector('.ecf-chip.on');
    ...
    document.getElementById('ecf-sticky-bar').style.display = 'none';
    setEnrollProgress(2);
    renderCoursePickerRows(filtered);
}

function renderCoursePickerRows(courses) {
    var emptyEl = document.getElementById('ecf-course-empty');
    var tblEl = document.getElementById('ecf-course-table');
    if (courses.length === 0) {
    ...
    pickerListEl.innerHTML = pageItems.map(function(c) {
        ...
        var badges = '';
        if (c.is_package) badges += '<span class="ecf-tag ecf-tag-pkg">小课包</span>';
        if (c.is_toddler) badges += '<span class="ecf-tag ecf-tag-toddler">幼儿</span>';
        return '<tr class="' + (selected ? 'sel' : '') + '" onclick="selectEnrollCourse(' + c.id + ', \'' + esc(c.name) + '\')">' +
            '<td class="col-radio">' + (selected ? '●' : '○') + '</td>' +
            '<td class="col-name" style="font-weight:600">' + esc(c.name) + badges + '</td>' +
            '<td class="col-type" style="font-size:12px;color:#6b6580">' + esc(subjectText) + '</td>' +
...
function selectEnrollCourse(courseId, courseName) {
    ...
    // 高亮选中行
    document.querySelectorAll('#enroll-course-list tr').forEach(function(tr) { tr.classList.remove('sel'); });
    // 用 onclick 匹配找到对应行
    var rows = document.querySelectorAll('#enroll-course-list tr');
    for (var i = 0; i < rows.length; i++) {
        if (rows[i].getAttribute('onclick') && rows[i].getAttribute('onclick').indexOf('selectEnrollCourse(' + courseId + ',') !== -1) {
    ...
    enrollTransitionHide(document.getElementById('enroll-items-section'));
    document.getElementById('ecf-sticky-bar').style.display = 'none';
    var plansSection = document.getElementById('enroll-plans-section');
    plansSection.style.display = '';
    document.getElementById('ecf-plan-course-name').textContent = courseName;
    var listDiv = document.getElementById('enroll-plans-list');
    listDiv.innerHTML = '<tr><td colspan="6" style="color:#999;padding:12px;text-align:center;">加载中...</td></tr>';
    ...
    fetch(API_BASE + 'get_course_plans&course_id=' + courseId)
        ...
        .catch(() => {
            listDiv.innerHTML = '<tr><td colspan="6" style="color:#e74c3c;padding:12px;text-align:center;">加载失败</td></tr>';
        });
}
    ...
    document.querySelectorAll('#panel-work-records .sec-tab').forEach(tab => {
        tab.addEventListener('click', function() {
            ...
            if (targetId === 'tab-refund-records') {
                refundPage = 1;
                loadRefundRecords();
            } else if (targetId === 'tab-course-transfer-records') {
                courseTransferPage = 1;
                loadCourseTransferRecords();
...
// ==================== 转课记录（工作记录面板） ====================
let courseTransferPage = 1;
let courseTransferPageSize = 20;

/**
 * 加载转课记录列表
 */
async function loadCourseTransferRecords(page) {
    if (page) courseTransferPage = page;
    const campus = (document.getElementById('filter-ctr-campus')?.value || '').trim();
    const student = (document.getElementById('filter-ctr-student')?.value || '').trim();
    const dateFrom = document.getElementById('filter-ctr-date-from')?.value || '';
    const dateTo = document.getElementById('filter-ctr-date-to')?.value || '';
    const status = document.getElementById('filter-ctr-status')?.value || '';
    const params = {
        campus: campus,
        keyword: student,
        date_from: dateFrom,
        date_to: dateTo,
        status: status,
        page: courseTransferPage,
        page_size: courseTransferPageSize
    };
    try {
        const data = await api('list_course_transfer_records', params, 'GET');
        renderCourseTransferRecordsTable(data);
    } catch (e) {
        showToast('加载转课记录失败', 'error');
    }
}
 ...
 * 渲染转课记录表格
 */
function renderCourseTransferRecordsTable(data) {
    const tbody = document.querySelector('#table-course-transfer-records tbody');
    if (!tbody) return;
    const rows = data.data || [];
    if (rows.length === 0) {
        tbody.innerHTML = '<tr><td colspan="14" style="text-align:center;color:#999;padding:30px;">暂无转课记录</td></tr>';
    } else {
        tbody.innerHTML = rows.map(r => {
            const cross = (parseInt(r.is_cross_subject) || 0) ? '<span style="color:#e67e22;font-weight:600;">是</span>' : '否';
            const stat = r.status === '正常'
                ? '<span style="color:#27ae60;">正常</span>'
                : '<span style="color:#999;">已撤销</span>';
            const action = r.status === '正常'
                ? `<button class="btn btn-sm" style="color:#e74c3c;border:1px solid #e74c3c;padding:2px 8px;border-radius:4px;cursor:pointer;" onclick="revokeCourseTransfer(${r.id})">撤销</button>`
                : '<span style="color:#999;">已撤销</span>';
            const createdAt = r.created_at ? r.created_at.slice(0, 16) : '';
            return `<tr>
                <td>${esc(r.student_name || '-')}</td>
                <td>${esc(r.student_phone || '-')}</td>
                <td>${esc(r.student_no || '-')}</td>
                <td>${esc(r.campus || '-')}</td>
                <td>${esc(r.source_course_name || '-')}</td>
                <td>${r.transfer_lessons || 0}</td>
                <td>¥${Number(r.transfer_value || 0).toFixed(2)}</td>
                <td>${esc(r.target_course_name || '-')}</td>
                <td>${r.target_lessons || 0}</td>
                <td>¥${Number(r.target_value || 0).toFixed(2)}</td>
                <td>${cross}</td>
                <td>${createdAt}</td>
                <td>${stat}</td>
    ...
    // 分页
    const pag = document.getElementById('pagination-course-transfer-records');
    if (pag) {
        const total = data.total || 0;
        ...
        if (totalPages <= 1) {
            ...
            for (let i = 1; i <= totalPages; i++) {
                html += `<button${i === page ? ' class="active"' : ''} onclick="loadCourseTransferRecords(${i})">${i}</button>`;
            }
            pag.innerHTML = html;
        }
    }
 ...
 * 撤销转课记录
 */
function revokeCourseTransfer(recordId) {
    showCustomConfirm('确认撤销该转课记录？撤销后将恢复源课程课时。', async function() {
        try {
            const res = await fetch(API_BASE + 'revoke_course_transfer', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
 ...
 * 填充转课记录校区筛选下拉
 */
async function initCourseTransferCampusFilter() {
    const sel = document.getElementById('filter-ctr-campus');
    if (!sel) return;
    try {
        const data = await api('list_campuses', null, 'GET');
        const campuses = data.data || [];
        sel.innerHTML = '<option value="">全部校区</option>' +
            campuses.map(c => `<option value="${esc(c.name || c)}">${esc(c.name || c)}</option>`).join('');
    } catch (e) { /* ignore */ }
}

// ==================== 转课弹窗 ====================
let courseTransferSourceOrder = null;
let courseTransferTargetCourseId = null;
let courseTransferIsCrossSubject = false;

/**
 * 显示转课弹窗
 */
async function showCourseTransferModal(orderId, transferRecordId) {
    let sourceRow;
    if (transferRecordId) {
    ...
    let targetCourses = [];
    try {
        const resp = await fetch(API_BASE + 'get_campus_courses&campus=' + encodeURIComponent(campus) + '&exclude_ids=' + Array.from(enrolledCourseIds).join(','));
        const d = await resp.json();
        targetCourses = d.data || [];
    } catch(e) {
        showToast('加载目标课程失败', 'error');
        return;
    }
    ...
    overlay.id = 'modal-course-transfer-overlay';
    overlay.style.display = 'flex';
    overlay.innerHTML = `
        <div class="modal-content modal-course-transfer">
            <h3>转课 - <span id="ct-source-course-name">${esc(sourceRow.name)}</span></h3>
            <div id="ct-source-info" style="margin-bottom:12px;padding:8px 12px;background:#f7f8fa;border-radius:6px;font-size:13px;">
                校区：<span id="ct-campus">${esc(campus)}</span> | 剩余课时：<span id="ct-remaining">${remaining}</span> | 单课时价值：<strong id="ct-unit-value">¥${unitValue.toFixed(2)}</strong>
            </div>

            <label>转出课时数：</label>
            <input type="number" id="ct-transfer-lessons" min="2" max="${remaining}" value="2" step="2" oninput="updateTransferPreview()" style="width:100%;padding:8px;border:1px solid #d9d9d9;border-radius:6px;margin-top:4px;">
            <div style="margin-top:4px;font-size:13px;">转出价值：<strong id="ct-transfer-value">¥${unitValue.toFixed(2)}</strong></div>

            <label>目标课程：</label>
            <select id="ct-target-course" onchange="onTargetCourseChange()" style="width:100%;padding:8px;border:1px solid #d9d9d9;border-radius:6px;margin-top:4px;">
                <option value="">请选择目标课程</option>
                ${targetOptions}
            </select>

            <div id="ct-cross-section" style="display:none;margin-top:8px;">
                <label>转入课时数（跨学科可编辑）：</label>
                <input type="number" id="ct-target-lessons" min="2" step="2" value="2" oninput="updateTransferPreview()" style="width:100%;padding:8px;border:1px solid #d9d9d9;border-radius:6px;margin-top:4px;">
                <div style="margin-top:4px;font-size:13px;">转入价值：<strong id="ct-target-value">¥${unitValue.toFixed(2)}</strong></div>
            </div>

            <div class="modal-actions" style="margin-top:20px;display:flex;gap:12px;justify-content:flex-end;">
                <button class="btn btn-secondary" onclick="document.getElementById('modal-course-transfer-overlay').remove()">取消</button>
                <button class="btn btn-primary" id="btn-submit-transfer" onclick="submitCourseTransfer()">确认转课</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
function updateTransferPreview() {
    const source = courseTransferSourceOrder;
    if (!source) return;
    const ap = parseFloat(source.actual_price) || 0;
    const lc = parseInt(source.lesson_count) || 1;
    const unitValue = lc > 0 ? (ap / lc) : 0;
    const transferLessons = parseInt(document.getElementById('ct-transfer-lessons')?.value) || 0;
    const transferValue = unitValue * transferLessons;

    const transferValueEl = document.getElementById('ct-transfer-value');
    if (transferValueEl) transferValueEl.textContent = '¥' + transferValue.toFixed(2);

    if (courseTransferIsCrossSubject) {
        const targetLessons = parseInt(document.getElementById('ct-target-lessons')?.value) || transferLessons;
        const targetValueEl = document.getElementById('ct-target-value');
        if (targetValueEl) targetValueEl.textContent = '¥' + transferValue.toFixed(2);
        // 同步 target_lessons 到 value（确保跨学科时有有效值）
        if (!document.getElementById('ct-target-lessons')?.value) {
            const el = document.getElementById('ct-target-lessons');
            if (el) el.value = transferLessons;
        }
    }

    // 限制最大转出课时
    const remaining = parseInt(source.remaining_lessons) || 0;
    const input = document.getElementById('ct-transfer-lessons');
    if (input && transferLessons > remaining) {
 ...
 * 目标课程变更处理
 */
function onTargetCourseChange() {
    const sel = document.getElementById('ct-target-course');
    if (!sel) return;
    const selectedOption = sel.options[sel.selectedIndex];
    const isCross = selectedOption && selectedOption.dataset.cross === '1';
    courseTransferIsCrossSubject = !!isCross;
    courseTransferTargetCourseId = sel.value ? parseInt(sel.value) : null;

    const crossSection = document.getElementById('ct-cross-section');
    if (crossSection) {
        crossSection.style.display = isCross ? 'block' : 'none';
    }

    // 跨学科时：target_lessons 默认 = transfer_lessons
    if (isCross) {
        const transferLessons = parseInt(document.getElementById('ct-transfer-lessons')?.value) || 2;
        const targetLessonsEl = document.getElementById('ct-target-lessons');
        if (targetLessonsEl && !targetLessonsEl.value) {
            targetLessonsEl.value = transferLessons;
        }
    }
 ...
 * 提交转课
 */
async function submitCourseTransfer() {
    const source = courseTransferSourceOrder;
    if (!source) {
        showToast('请先选择源课程', 'error');
        return;
    }
    if (!courseTransferTargetCourseId) {
        showToast('请选择目标课程', 'error');
        return;
    }
    const transferLessons = parseInt(document.getElementById('ct-transfer-lessons')?.value) || 0;
    if (transferLessons <= 0) {
        showToast('转出课时数必须大于0', 'error');
        return;
    }
    const remaining = parseInt(source.remaining_lessons) || 0;
    if (transferLessons > remaining) {
        showToast('转出课时数不能超过剩余课时(' + remaining + ')', 'error');
        return;
    }

    const body = {
        source_order_id: parseInt(source.is_transfer_course ? (source.source_order_id || 0) : source.order_id),
        target_course_id: courseTransferTargetCourseId,
    ...
    try {
        const res = await fetch(API_BASE + 'create_course_transfer', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.success) {
            showToast('转课成功');
            // 关闭弹窗
            const overlay = document.getElementById('modal-course-transfer-overlay');
            ...
            showToast(data.message || '转课失败', 'error');
        }
    } catch (e) {
        ...
        <div class="resale-modal">
            ...
            <div class="resale-modal-body">
                <div class="seller-info">
                    ...
                    <div><strong>剩余课时：</strong>${remaining} | <strong>剩余课时价值：</strong>¥${(unitValue * remaining).toFixed(2)} | <strong>单课时价值：</strong>¥${unitValue.toFixed(2)}</div>
                </div>
...
async function loadTransferRecords(page) {
    if (page) transferPage = page;
    const status = document.getElementById('filter-transfer-status')?.value || '';
    const search = document.getElementById('filter-transfer-search')?.value || '';
    const params = new URLSearchParams({ action: 'list_transfer_records', page: transferPage, page_size: transferPageSize });
    if (status) params.set('status', status);
    ...
    try {
        ...
        renderTransferRecords(data.data || [], data.total || 0);
    } catch (e) {
        console.error('加载转校记录失败', e);
    }
}
...
async function showTransferModal(orderId, transferId = 0, courseTransferRecordId = 0) {
    ...
    try {
        const resp = await fetch(`?action=list_campuses`);
        const data = await resp.json();
        ...
        <div class="modal" style="max-width:520px;">
            ...
            <div class="modal-body">
                <table style="width:100%;margin-bottom:16px;border-collapse:collapse;">
                    <tr><td style="padding:6px 12px;background:#f5f5f5;width:100px;">课程名称</td><td style="padding:6px 12px;">${esc(orderRow.name)}</td></tr>
                    <tr><td style="padding:6px 12px;background:#f5f5f5;">当前校区</td><td style="padding:6px 12px;">${esc(orderRow.campus || '-')}</td></tr>
                    <tr><td style="padding:6px 12px;background:#f5f5f5;">剩余课时</td><td style="padding:6px 12px;font-weight:bold;color:#1890ff;">${remainingLessons} 课时</td></tr>
                    <tr><td style="padding:6px 12px;background:#f5f5f5;">剩余金额</td><td style="padding:6px 12px;font-weight:bold;color:#1890ff;">${remainingAmount.toFixed(2)} 元</td></tr>
                </table>
                <div class="form-group">
                    <label>目标校区</label>
                    <select id="transfer-to-campus" class="form-control" onchange="onTransferCampusChange()">
                        ${campusOptions}
                    </select>
                </div>
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-primary" id="btn-submit-transfer" onclick="submitTransfer()" disabled>提交申请</button>
            </div>
        </div>
    </div>`;
...
async function showRefundApplyModal(orderId, transferId = 0, courseTransferRecordId = 0) {
    // 从当前学员课程数据中找到目标订单
    let orderRow;
    if (courseTransferRecordId > 0) {
        orderRow = studentCoursesAllRows.find(r => r.is_transfer_course && r.transfer_record_id == courseTransferRecordId);
    ...
    document.getElementById('refund-apply-ctr-id').value = courseTransferRecordId;
...
async function submitRefundApply() {
    ...
    try {
        ...
        } else if (courseTransferRecordId > 0) {
            body.course_transfer_record_id = courseTransferRecordId;
            body.order_id = orderId;  // 底层订单ID（用于查找源订单）
        } else {
...
function switchTaTab(tab) {
    // 更新 tab 按钮状态
    document.querySelectorAll('#ta-tabs .sec-tab').forEach(btn => btn.classList.remove('active'));
    document.querySelectorAll('#ta-tabs .sec-tab').forEach(btn => {
        if (btn.textContent.includes(tab === 'list' ? '画具列表' : tab === 'purchase' ? '购买画具' : '销售记录')) {
            btn.classList.add('active');
        }
    });
    // 切换面板
    document.querySelectorAll('#panel-teaching-aids .sec-panel').forEach(p => p.classList.remove('active'));
    const panelId = tab === 'list' ? 'tab-ta-list' : tab === 'purchase' ? 'tab-ta-purchase' : 'tab-ta-sales';
...
async function searchPurchaseStudent(keyword) {
    const dropdown = document.getElementById('ta-student-dropdown');
...
async function loadStudentBalance() {
    if (!taPurchaseStudent) return;
    const data = await api('get_student_account', { student_id: taPurchaseStudent.id }, 'GET');
...
function changeCartQty(aid, delta) {
    if (!taCartItems[aid]) return;
    taCartItems[aid].quantity = Math.max(1, taCartItems[aid].quantity + delta);
    renderCart();
    updatePurchaseBtn();
}

function renderCart() {
    const cartContainer = document.getElementById('ta-cart-items');
    const summaryDiv = document.getElementById('ta-cart-summary');
    ...
    let total = 0;
    cartContainer.innerHTML = cartKeys.map(aid => {
        const item = taCartItems[aid];
        const subtotal = item.price * item.quantity;
        total += subtotal;
        return `
            <div class="ta-cart-item">
                <div class="ta-cart-item-info">
...
function updatePurchaseBtn() {
    ...
    const cash = parseFloat(document.getElementById('ta-pay-cash').value) || 0;
    const meituan = parseFloat(document.getElementById('ta-pay-meituan').value) || 0;
    const account = parseFloat(document.getElementById('ta-pay-account').value) || 0;
    const matchDiv = document.getElementById('ta-payment-match');
    const matchText = document.getElementById('ta-payment-match-text');
    const btn = document.getElementById('ta-purchase-btn');
    
    const hasStudent = taPurchaseStudent !== null;
    const hasItems = Object.keys(taCartItems).length > 0;
    const paymentSum = cash + meituan + account;
    const isMatch = Math.abs(paymentSum - total) < 0.005;
    ...
    if (hasStudent && hasItems && total > 0) {
        if (isMatch) {
            ...
            btn.disabled = true;
            btn.title = '支付金额与商品总价不匹配';
        }
    } else {
        matchDiv.style.display = 'none';
        btn.disabled = true;
        btn.title = hasStudent ? (hasItems ? '支付金额不匹配' : '请选择商品') : '请先选择学员';
    }
}
...
async function confirmPurchase() {
    ...
    const items = Object.values(taCartItems).map(item => ({
        teaching_aid_id: item.id,
...
async function loadTeachingAidSales(page = 1, ps = null) {
    ...
    const dateFrom = document.getElementById('ta-sales-date-from')?.value || '';
...
document.addEventListener('click', function(e) {
    const dropdown = document.getElementById('ta-student-dropdown');
    const searchInput = document.getElementById('ta-purchase-student-search');
...
function selectEnrollType(type) {
    ...
    if (type === 'course') {
        ...
        activityEnrollState = { campus: campusName, activity: null, adultCount: 0, studentCount: 0, step: 0, adultDeductions: [], studentDeductions: [] };
        ...
        document.getElementById('enroll-activity-step-subject').style.display = 'none';
        document.getElementById('enroll-activity-advisor-section').style.display = 'none';
        document.getElementById('enroll-activity-pay-section').style.display = 'none';
        document.getElementById('enroll-activity-confirm-bar').style.display = 'none';
...
function selectActivityCampus(campusName) {
    ...
    currentEnrollCampusName = campusName;
...
async function loadActivityList(campusName) {
    ...
    // 加载态必须用 tr+td 包裹，否则浏览器会自动把 div 提到 tbody 外面、破坏表格结构
    listDiv.innerHTML = '<tr id="activity-loading-row"><td colspan="6" style="text-align:center;color:#999;padding:20px;">加载中…</td></tr>';
    ...
    try {
        ...
        listDiv.innerHTML = '<tr><td colspan="6" style="text-align:center;color:#e74c3c;padding:20px;">加载活动失败: ' + esc(e.message) + '</td></tr>';
    }
}
...
function renderActivityCards(activities) {
    ...
    listDiv.innerHTML = activities.map(a => {
        ...
        else if (feeStudent > 0) feeText = '¥' + feeStudent.toFixed(0) + '/人';
        else if (feeAdult > 0) feeText = '¥' + feeAdult.toFixed(0) + '/人';
        else feeText = '免费';
        const dotColor = full ? 'enroll-cap-dot-err' : (remaining <= 5 ? 'enroll-cap-dot-warn' : 'enroll-cap-dot-ok');
        const capText = full ? '已满员' : ('余 ' + remaining + ' (已报' + enrolled + '/' + capacity + ')');
        const hasDed = (a.student_fee_mode === 'fee_and_deduct' || a.student_fee_mode === 'deduct_only');
        let deductText = '';
        if (hasDed && a.deductions && a.deductions.student && a.deductions.student.length > 0) {
            deductText = a.deductions.student.map(function(d) { return (d.subject_level1 || '默认') + ' ' + (d.deduct_lessons || 1) + '课时'; }).join('；');
        } else if (hasDed) {
            deductText = '按学科扣除';
        } else {
            deductText = '—';
        ...
        const dedRules = a.deductions && a.deductions.student ? JSON.stringify(a.deductions.student) : '[]';
        return '<tr ' + cls + ' data-activity-id="' + a.id + '" data-activity-name="' + esc(a.name) + '" data-fee-adult="' + feeAdult + '" data-fee-student="' + feeStudent + '" data-subject="' + esc(a.subject_level1 || '') + '" data-deduct-rules=\'' + dedRules + '\' data-has-subject-deduction="' + (hasDed ? 1 : 0) + '" data-remaining="' + remaining + '" onclick="selectActivity(' + a.id + ', \'' + esc(a.name).replace(/'/g, "\\'") + '\', this)"><td style="color:var(--color-primary);font-weight:700;text-align:center">●</td><td>' + esc(a.name) + '</td><td>' + esc(a.subject_level1 || '') + '</td><td>' + feeText + '</td><td><span class="enroll-cap-dot ' + dotColor + '"></span>' + capText + '</td><td>' + deductText + '</td></tr>';
    }).join('');
}
...
function selectActivity(activityId, activityName, cardEl) {
    console.log('[selectActivity] called. currentEnrollStudentId=', currentEnrollStudentId, 'currentEnrollCampusName=', currentEnrollCampusName);
    ...
    var remaining = parseInt(cardEl.dataset.remaining) || 0;
    if (remaining <= 0) { showToast('该活动名额已满', 'error'); return; }
    activityEnrollState.activity = { id: activityId, name: activityName, feeAdult: parseFloat(cardEl.dataset.feeAdult) || 0, feeStudent: parseFloat(cardEl.dataset.feeStudent) || 0, subject: cardEl.dataset.subject, deductionRules: ded, adultDeductions: ded.adult || [], studentDeductions: ded.student || [], hasSubjectDeduction: hasDed, remaining: remaining };
    // highlight selected row
    document.querySelectorAll('#activity-card-list tr').forEach(function(r) { r.classList.toggle('selected', false); });
    ...
    document.getElementById('enroll-activity-step-subject').style.display = '';
    document.getElementById('enroll-activity-advisor-section').style.display = '';
    document.getElementById('enroll-activity-pay-section').style.display = '';
    document.getElementById('enroll-activity-confirm-bar').style.display = '';
    // init count
    ...
    recalcActivityTotal();
    // render payment summary & fill default
    renderActivityPaymentSummary();
    // load subject hours & init attendance radio
    loadStudentSubjectHours();
    // init advisor dropdown
    initActivityAdvisorDropdown();
}

async function loadStudentSubjectHours() {
    var sid = currentEnrollStudentId;
    var campus = currentEnrollCampusName || activityEnrollState.campus;
    var container = document.getElementById('activity-subject-hours');
    console.log('[loadStudentSubjectHours] sid=', sid, 'campus=', campus, 'container=', !!container);
    if (!container) {
        return;
    }
    container.innerHTML = '<div style="text-align:center;color:#999;padding:8px;">加载中...</div>';
    if (!sid) {
        container.innerHTML = '<div style="text-align:center;color:#e74c3c;padding:8px;">学员未选定，请返回学员列表重新点击报名</div>';
        return;
    }
    if (!campus) {
        container.innerHTML = '<div style="text-align:center;color:#e74c3c;padding:8px;">校区未选定，请先选择校区</div>';
        return;
    }
    try {
        var res = await fetch(API_BASE + 'get_student_subject_hours&student_id=' + sid + '&campus_name=' + encodeURIComponent(campus));
        var data = await res.json();
        if (data.error) {
            container.innerHTML = '<div style="text-align:center;color:#e74c3c;padding:8px;">查询失败：' + escHtml(data.error) + '</div>';
            return;
        }
        var list = data.data || [];
        if (list.length === 0) {
            container.innerHTML = '<div style="text-align:center;color:#999;padding:16px;background:#f9f9f9;border-radius:6px;">该学员在本校区暂无有效课时</div>';
            return;
        }
        var html = '<table class="subject-hours-table" style="width:100%;border-collapse:collapse;font-size:14px;">';
        html += '<thead><tr style="background:#f5f7fa;text-align:left;"><th style="padding:8px 12px;">一级学科</th><th style="padding:8px 12px;text-align:right;">剩余课时</th><th style="padding:8px 12px;text-align:center;width:100px;">优先扣减</th></tr></thead><tbody>';
        for (var i = 0; i < list.length; i++) {
            var item = list[i];
            var subj = item.subject || '未分类';
            var rem = parseInt(item.remaining) || 0;
            html += '<tr onclick="pickActivitySubject(this)" style="cursor:pointer"><td style="padding:8px 12px;border-bottom:1px solid #eee;">' + escHtml(subj) + '</td><td style="padding:8px 12px;text-align:right;font-weight:600;border-bottom:1px solid #eee;">' + rem + ' 课时</td><td style="padding:8px 12px;text-align:center;border-bottom:1px solid #eee;"><input type="radio" name="activity-att-subj" value="' + escHtml(subj) + '"></td></tr>';
        }
        html += '</tbody></table>';
        container.innerHTML = html;
    } catch (e) {
        container.innerHTML = '<div style="text-align:center;color:#e74c3c;padding:8px;">请求失败：' + (e.message || '') + '</div>';
    }
}

function pickActivitySubject(row) {
    document.querySelectorAll('.subject-hours-table tbody tr').forEach(function(r) { r.classList.remove('att-sel'); });
    row.classList.add('att-sel');
    var radio = row.querySelector('input[type="radio"]');
    if (radio) radio.checked = true;
}

function getActivityAttendanceSubject() {
    var checked = document.querySelector('input[name="activity-att-subj"]:checked');
    return checked ? checked.value : '';
}

function nextActivityStep() { /* 单页模式，无分步导航 */ }
function prevActivityStep() { /* 单页模式，无分步导航 */ }
function updateActivityStepButtons() { /* 单页模式，按钮常驻 */ }

function resetActivityEnroll() {
    activityEnrollState.activity = null;
    document.querySelectorAll('#activity-card-list tr').forEach(function(r) { r.classList.remove('selected'); });
    document.getElementById('enroll-activity-step-count').style.display = 'none';
    document.getElementById('enroll-activity-step-subject').style.display = 'none';
    document.getElementById('enroll-activity-advisor-section').style.display = 'none';
    document.getElementById('enroll-activity-pay-section').style.display = 'none';
    document.getElementById('enroll-activity-confirm-bar').style.display = 'none';
    document.getElementById('activity-card-list').scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function initActivityAdvisorDropdown() {
    var el = document.getElementById('dropdown-activity-advisor');
    if (!el) { console.warn('[advisor] dropdown-activity-advisor not found'); return; }
    if (window._activityAdvisorDD) {
        try { window._activityAdvisorDD.reset(); } catch(e) {}
    }
    var campusName = activityEnrollState.campus || currentEnrollCampusName || '';
    try {
        var dd = new SearchableDropdown(el, 'employee');
        dd.setCampus(campusName);
        dd.loadData().catch(function(err) { console.error('[advisor] loadData failed:', err); });
        window._activityAdvisorDD = dd;
    } catch (e) {
        console.error('[advisor] init failed:', e);
        var input = el.querySelector('.searchable-input');
        if (input) { input.disabled = true; input.placeholder = '顾问加载失败'; }
    }
}
...
function renderActivityPaymentSummary() {
    ...
    const cashInput = document.getElementById('activity-pay-cash');
...
async function confirmActivityEnroll() {
    const total = calcActivityTotal();
    if (!activityEnrollState.activity) { showToast('请先选择活动', 'error'); return; }
    if (!currentEnrollStudentId) { showToast('请先选择学员', 'error'); return; }
    // read counts from inputs
    var adultCount = parseInt(document.getElementById('activity-adult-count')?.value) || 0;
    var studentCount = parseInt(document.getElementById('activity-student-count')?.value) || 0;
    if (adultCount + studentCount <= 0) { showToast('请填写报名人数', 'error'); return; }
    if (adultCount + studentCount > activityEnrollState.activity.remaining) { showToast('剩余名额不足', 'error'); return; }
    const cash = parseFloat(document.getElementById('activity-pay-cash')?.value) || 0;
    const meituan = parseFloat(document.getElementById('activity-pay-meituan')?.value) || 0;
    const balance = parseFloat(document.getElementById('activity-pay-balance')?.value) || 0;
    const online = parseFloat(document.getElementById('activity-pay-online')?.value) || 0;
    ...
    try {
        var advisorId = (window._activityAdvisorDD && window._activityAdvisorDD.getValue) ? window._activityAdvisorDD.getValue() : 0;
        const payload = { student_id: currentEnrollStudentId, activity_id: activityEnrollState.activity.id, campus_name: activityEnrollState.campus, adult_count: adultCount, student_count: studentCount, payment_cash: cash, payment_meituan: meituan, use_balance: balance > 0 ? 1 : 0, balance_amount: balance, payment_online: online, payment_tonglian: tonglian, payment_zhishouyin: zhishouyin, payment_douyin: douyin, attendance_subject: getActivityAttendanceSubject(), advisor_id: advisorId };
...
class SearchableDropdown {
    ...
    async loadData() {
        if (this.type === 'employee') {
            const res = await api('search_employees', { campus: this.campusName }, 'GET');
            if (res.data) this.items = Object.values(res.data);
        } else {
            const res = await api('search_students', null, 'GET');
            if (res.data) this.items = Object.values(res.data);
        }
        this._sortItems();
